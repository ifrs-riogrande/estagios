...
(function () {
  ...
  function _injectStyles() {
    ...
    style.textContent = [
      ...
      '.sge-bell-btn{position:relative;background:none;border:none;cursor:pointer;padding:6px;border-radius:8px;color:var(--color-text-secondary,#6b7280);display:flex;align-items:center;justify-content:center;transition:background .15s,color .15s}',
      ...
      '.sge-bell-list{max-height:320px;overflow-y:auto}',
  ...
  function _renderList() {
    ...
    list.innerHTML = _notifs.map(function (n) {
      ...
      return '<div class="sge-bell-item" data-id="' + _esc(n.id) + '" data-estagio="' + _esc(n.idEstagio) + '">' +
        '<div class="sge-bell-dot"></div>' +
        '<div class="sge-bell-item-body">' +
          '<div class="sge-bell-item-msg">' + _esc(n.mensagem) + '</div>' +
          '<div class="sge-bell-item-meta">' + (n.idEstagio ? n.idEstagio + ' · ' : '') + dt + '</div>' +
        '</div>' +
      '</div>';
    }).join('');

    list.querySelectorAll('.sge-bell-item').forEach(function (el) {
  ...
  function _carregar() {
    ...
    API.get('listarNotificacoes', { authToken: token })
      .then(function (data) {
      ...
      .catch(function () {
        if (list) list.innerHTML = '<div class="sge-bell-empty">Erro ao carregar notificações.</div>';
      });
  }
